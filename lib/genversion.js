...
exports.dry = (targetPath, opts, callback) => {
  ...
  //       withGit:
  //         boolean. Add git revision and repo status to the version.
  //   callback
  ...
  if (typeof opts.withGit !== 'boolean') {
    opts.withGit = false // default
  }
  ...
  if (opts.withGit) {
    const githash = git.githash();
    const gitclean = git.gitclean();
    version += "+" + githash + gitclean;
  }
